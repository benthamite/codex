...
The =app-server= backend renders conversation items the way the Codex CLI does.  Command executions are collapsed exactly like the CLI: a ~• Ran COMMAND~ header (or ~✗ Ran COMMAND (exit N)~ on failure), then the first ~codex-app-server-max-command-output-lines~ output lines (default 3, matching the CLI), a ~… +N lines~ marker, and the final output line, indented under a ~└~ connector.  This avoids dumping full command output (which can be megabytes) into the buffer; the full output is available with ~codex-app-server-expand-output~ (=C-c C-o=).  Reasoning summaries render as dimmed text, file changes render as folded diffs with added and removed line faces, MCP tool calls show the tool and folded result, web searches show the query, and the turn plan renders as a checklist that updates in place as steps complete.
