...
Command executions in =app-server= buffers are folded like the Codex CLI: the command line is shown, followed by at most ~codex-app-server-max-command-output-lines~ lines of output and a ~… +N lines~ marker for the remainder.  This avoids dumping full command output (which can be megabytes) into the buffer.  The default limit is 10 lines.
