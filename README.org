...
On the =app-server= backend, slash commands typed into the input are dispatched locally instead of being sent to the model: ~/compact~ compacts the conversation, ~/clear~ clears the display, ~/status~ shows model and token usage, ~/diff~ shows the working-tree diff, ~/copy~ copies the last reply, ~/new~, ~/resume~, ~/fork~, ~/permissions~, ~/quit~ map to the matching commands, and ~/init~ and ~/review~ are sent as prompts.
