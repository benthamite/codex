...
The package runs Codex through the native =app-server= renderer or a terminal backend (=eat= by default, with =vterm= available separately), giving you a full-featured Codex experience without leaving your editor.  You can start, stop, and manage multiple Codex instances across different projects, send commands and code from any buffer, toggle the Codex window on and off, and access common CLI slash commands through a transient menu.  Terminal backends pass those commands directly to the Codex TUI.  The =app-server= backend exposes a command only when it can invoke the corresponding Codex protocol operation or provide the same user-visible frontend action; it does not replace Codex commands with configuration notices or unrelated Emacs behavior.
...
On the =app-server= backend, slash commands typed into the input are handled by the frontend instead of being sent to the model.  Protocol-backed commands invoke the same Codex app-server methods used by native clients: ~/skills~ lists skills and can insert or enable/disable one; ~/plugins~ lists, installs, enables/disables, or uninstalls plugins; ~$~ in the composer starts a mention, mirroring the CLI, which binds ~$~ the same way it binds ~@~ for files: the character is typed into the composer and the candidates narrow there as you keep typing, rather than through a minibuffer prompt.  The candidates are the installed and enabled plugins, annotated =[Plugin]=, followed by the enabled skills, annotated =[Skill]=, both fetched from the server when the thread starts and matched on any part of the identifier, as the CLI matches them.  Completing inserts the identifier, so ~$brow~ completes to ~$browser~ and ~$brainst~ to ~$superpowers:brainstorming~; ~/hooks~ lists hooks and can enable, disable, or trust one; ~/experimental~ lists and persistently toggles features; ~/mcp~, ~/ps~, ~/debug-config~, and ~/usage~ read their corresponding Codex inventories; ~/permissions~ lists and applies a Codex permission profile; ~/feedback~ uploads feedback; ~/logout~ logs out after confirmation; ~/fast~ persistently updates the service tier and applies it to the current thread; ~/stop~ lists and terminates background terminals; and ~/delete~ deletes the current thread after confirmation.  Thread and model commands such as ~/compact~, ~/model~, ~/goal~, ~/goal-clear~, ~/rename~, ~/archive~, ~/unarchive~, ~/memories~, and ~/personality~ likewise use their matching app-server requests.  Commands whose native behavior belongs specifically to the terminal TUI, or for which the app-server exposes no equivalent operation, are not advertised by app-server completion and are reported as unsupported rather than being replaced by an Emacs-specific action.  Frontend actions with the same observable result remain available: ~/clear~ and ~/new~ start a new conversation, ~/diff~ shows the agent's own changes for the thread rather than the working tree, matching the CLI, which reports no changes when the only edits present are the user's own; ~/copy~ copies the last reply, ~/resume~ and ~/fork~ use the native thread requests, ~/mention~ attaches a file, ~/raw~ toggles rendered versus raw output, ~/init~ submits the same task prompt, and ~/review~ starts a native review through ~review/start~, offering the CLI's four presets: a base branch, uncommitted changes, a commit, or custom instructions.  Lifecycle hook events (~hook/started~, ~hook/completed~) render as dimmed status lines when ~codex-app-server-show-hooks~ is enabled, and primary rate-limit usage is recorded and shown by ~/status~, alongside the active account's email address and plan, which the backend reads once when the thread starts.  The app-server backend reads account rate limits during startup; when weekly usage is high, the CLI-style weekly-limit warning renders above the idle composer.  Other warnings, deprecation and guardian notices, conversation compaction, model reroutes, MCP server startup progress and failures, and thread name and goal updates also render as status lines.

Extensions can use ~codex-app-server-turn-completed-hook~ after a turn becomes idle and before Codex submits queued input.
...
Images can be attached to a turn on the =app-server= backend: ~codex-default-images~ are attached to the first prompt, ~codex-app-server-attach-image~ attaches an image file to the next turn, and ~codex-app-server-paste-image~ (bound to ~C-v~, matching the Codex CLI's Ctrl+V) attaches an image directly from the system clipboard and inserts an ~[Image #N]~ placeholder into the composer at point, exactly as the CLI does.  Attached images are sent as =localImage= input items in attachment order.  Text, images, and file mentions remain one submission while queued or awaiting server acceptance; a rejected send restores them to the composer.  Clipboard image temporary files stay available until the accepted turn finishes, because the app-server worker reads them after acknowledging the request, and are also removed when the buffer is cleaned up.  User-selected image files are never removed.

Assistant messages in =app-server= buffers are rendered as Markdown while they stream and after they complete, the way the Codex CLI renders them.  Partial streaming fragments are isolated from =markdown-mode='s end-of-buffer assumptions, so a delta that ends at an opening code fence does not interrupt the session or add text to the message.  When =markdown-mode= is available, codex.el fontifies each message through =gfm-mode= and, like the CLI, hides only the inline emphasis markers (so =**bold**= and =`code`= show as styled text without the delimiters) while keeping block markers such as heading =#= and list =-= visible and syntax-coloured.  The user option ~codex-app-server-render-markdown~ enables this rendering; the default is ~t~.  When it is nil, or when =markdown-mode= is not installed, a lightweight built-in highlighter is used instead.
...
The user option ~codex-remap-light-backgrounds~ is the master switch.  The default is ~t~.  Its name is historical: the remapper handles both light CLI backgrounds on dark Emacs themes and dark CLI backgrounds on light Emacs themes.  When non-nil, background colors whose WCAG contrast against the Emacs default background exceeds ~codex-background-contrast-threshold~ are stripped (or replaced), and --- when ~codex-minimum-contrast-ratio~ is also non-nil --- low-contrast foregrounds are stripped as well.
...
The user option ~codex-vterm-buffer-multiline-output~ controls whether multi-line terminal output is buffered to prevent flickering.  The default is ~t~.  When enabled, rapid successive output chunks containing cursor movement sequences are accumulated and flushed as a single update, including the final buffered chunk when the process exits.
...
The function ~codex-start-session~ starts a Codex session from explicit keyword parameters (~:directory~, ~:instance-name~, ~:initial-prompt~, ~:resume-id~, ~:fork~, ~:terminal-backend~) and returns its buffer. Passing ~:fork~ alongside ~:resume-id~ forks that session into a new one rather than continuing it, leaving the original untouched: the terminal backends run =codex fork= with the session id, while the app-server backend forks natively through ~thread/fork~. This is what lets a caller branch a session whose id it already knows, without the interactive thread picker that ~codex-fork~ opens.
...
Before user hook handlers run, ~codex-handle-hook~ also updates the target buffer's transcript metadata.  When hook JSON includes an explicit transcript path, that path is recorded directly and its filename UUID takes precedence over a conflicting session ID in the hook payload.  When only a session id is available, codex.el looks up the matching JSONL file under ~codex-transcript-sessions-directory~ and caches successful lookups so later hooks for the same session do not rescan the whole sessions tree.  Cached paths whose filename UUID does not match the requested session are discarded before lookup.  If ~codex-transcript-catch-up-on-stop~ is enabled, Stop events can repair the final recorded agent message internally; by default this automatic repair is disabled to avoid racing the transcript writer.
...
The transient ~codex-slash-commands~ provides quick access to common Codex CLI slash commands, organized into five groups: Core (help, clear, compact, status, new, quit), Navigation & Review (diff, review, fork, resume, copy), Configuration (permissions, model, fast, plan, init, statusline, theme, debug config), Features & Tools (experimental, MCP, agent, apps, skills, plugins, hooks, mention, ps), and Account & Identity (logout, personality, feedback, usage, delete thread).  Terminal backends submit the literal slash command to the Codex TUI.  On the =app-server= backend, the command goes through the equivalent protocol-backed dispatcher described above.
...
This release improves Codex terminal and app-server robustness in Emacs.
Programmatic =eat= submissions now send prompt text literally, live-instance
selection ignores stale Codex-named buffers whose terminal process has exited,
transcript lookup results are cached by session id, and retained terminal
output is protected across redraw and escape-sequence edge cases.  App-server
submissions keep their attachments through queues and failed requests, process
exit clears transient working state, reasoning controls follow each model's
advertised levels, protocol notification fields and added-file diffs follow the
current schema, and skill/plugin changes refresh mention completion.
