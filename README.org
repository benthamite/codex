...
- *Native rendering and TUI fallback* --- render new Codex sessions from app-server protocol events so assistant messages, Markdown tables, and post-table text are not dependent on terminal screen history; keep the Eat and vterm terminal TUI backends available when needed.
...
The package requires Emacs 28.1 or later, =transient= 0.9.3 or later, =inheritenv=, and =eat=.  The default backend is =app-server=, which talks to the Codex app-server protocol and renders conversation events directly in Emacs.  You can still use the terminal TUI through =eat=, or through =vterm= by installing it separately and setting ~codex-terminal-backend~ to ~vterm~.
...
The user option ~codex-terminal-backend~ selects the Codex backend.  The choices are ~app-server~ (the default), ~eat~, and ~vterm~.  The =app-server= backend starts ~codex app-server --listen stdio://~, parses its newline-delimited JSON-RPC event stream, and renders assistant messages directly in the Emacs buffer.  This avoids terminal redraw and scrollback loss for wide Markdown tables.  The =eat= and =vterm= choices keep the Codex terminal TUI available as fallback backends.  On the =app-server= backend, ~codex-resume~ and ~codex-fork~ work natively: they list previous threads with =thread/list=, prompt for one, resume or fork it through =thread/resume= or =thread/fork=, and replay the prior conversation history into the buffer.

When an =app-server= session connects, codex.el renders a session header showing the Codex version, the model or provider, the working directory, the thread id, and the session transcript path.  The header replaces the terminal TUI's startup banner, which is not emitted over the app-server protocol.

The =app-server= buffer presents an editable input region after a prompt at the end of the buffer.  Type a message there and press the send key to submit it; conversation output renders above the prompt while the rendered history stays read-only and fully scrollable.  The send and newline keys follow ~codex-newline-keybinding-style~, so a multi-line prompt is composed with the newline key and submitted with the send key.  As in the Codex CLI, sending while a turn is active steers the current turn.  Pressing =TAB= completes a slash command when idle and queues the input to run as the next turn while a turn is in progress.  Several composer affordances match the CLI: a leading =!= runs the rest of the line as a shell command in the thread, typing =@= completes a project file path into the message, =M-p= and =M-n= cycle through previously sent prompts (with =C-c C-r= searching them by completion), and =C-c C-e= opens a separate buffer to compose a long prompt that is sent back with =C-c C-c=.  The user option ~codex-app-server-prompt-string~ sets the prompt shown before the input region; the default is ~"\n❯ "~.

The =app-server= backend renders conversation items the way the Codex CLI does.  Command executions are folded: the command line is shown, followed by at most ~codex-app-server-max-command-output-lines~ lines of output, a ~… +N lines~ marker for the remainder, and a status line with the exit result and duration.  This avoids dumping full command output (which can be megabytes) into the buffer; the default limit is 10 lines.  Reasoning summaries render as dimmed text, file changes render as folded diffs with added and removed line faces, MCP tool calls show the tool and folded result, web searches show the query, and the turn plan renders as a checklist that updates in place as steps complete.

When Codex requests approval for a command or file change, codex.el prompts with the same choices as the CLI: approve once, approve for the rest of the session, decline, or cancel the turn.

While a turn is running, the mode line shows a working indicator with elapsed time, token usage, and an Esc-to-interrupt hint, mirroring the CLI status line.

On the =app-server= backend, slash commands typed into the input are dispatched locally instead of being sent to the model: ~/compact~ compacts the conversation, ~/clear~ clears the display, ~/status~ shows model and token usage, ~/diff~ shows the working-tree diff, ~/copy~ copies the last reply, ~/new~, ~/resume~, ~/fork~, ~/permissions~, ~/quit~ map to the matching commands, ~/model~ lists and switches the model through =thread/settings/update=, ~/mention~ attaches a file, ~/agent~ switches between Codex buffers, ~/side~ starts a new session, ~/theme~ and ~/vim~ map to the Emacs theme and Vim-mode equivalents, ~/keymap~ describes the buffer keymap, ~/raw~ toggles Markdown rendering, ~/goal~, ~/title~, ~/memories~, ~/personality~, ~/plan~, and ~/stop~ drive the matching =thread/*= requests, and ~/init~ and ~/review~ are sent as prompts.  Every other documented slash command (~/fast~, ~/mcp~, ~/ps~, ~/ide~, ~/logout~, ~/experimental~, ~/plugins~, ~/hooks~, ~/approve~, ~/debug-config~, ~/feedback~) is recognized and reports how it is configured, so no slash command is sent to the model as text.  Lifecycle hook events (~hook/started~, ~hook/completed~) render as dimmed status lines when ~codex-app-server-show-hooks~ is enabled, and rate-limit usage is recorded and shown by ~/status~.  Warnings, deprecation and guardian notices, conversation compaction, model reroutes, MCP server startup status, and thread name and goal updates also render as status lines.
...
Images can be attached to a turn on the =app-server= backend: ~codex-default-images~ are attached to the first prompt, ~codex-app-server-attach-image~ attaches an image file to the next turn, and ~codex-app-server-paste-image~ (bound to ~C-c C-v~) attaches an image directly from the system clipboard.  Attached images are sent as =localImage= input items.

Completed assistant messages in =app-server= buffers are rendered as Markdown.  When =markdown-mode= is available, codex.el renders each message through =gfm-mode= and hides the Markdown markup, so headings, bold and italic spans, strikethrough, inline code, fenced code blocks, lists, blockquotes, links, and horizontal rules display as formatted text while the raw Markdown remains in the buffer for copying.  The user option ~codex-app-server-render-markdown~ enables this rendering; the default is ~t~.  When it is nil, or when =markdown-mode= is not installed, a lightweight built-in highlighter is used instead.
