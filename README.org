...
Before user hook handlers run, ~codex-handle-hook~ also updates the target buffer's transcript metadata.  If ~codex-transcript-catch-up-on-stop~ is enabled, Stop events can repair the final recorded agent message internally; by default this automatic repair is disabled to avoid racing the transcript writer.
