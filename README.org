...
Images can be attached to a turn on the =app-server= backend: ~codex-default-images~ are attached to the first prompt, and ~codex-app-server-attach-image~ attaches an image file to the next turn.  Attached images are sent as =localImage= input items.
