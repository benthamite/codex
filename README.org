...
#+vindex: codex-transcript-catch-up-delay
...
The user option ~codex-transcript-catch-up-on-stop~ controls whether Stop hooks repair stale terminal output from Codex JSONL transcripts.  The default is ~t~ because Codex's terminal renderer can omit assistant output that remains present in the authoritative transcript, especially around Markdown tables.  Stop-hook repair waits ~codex-transcript-catch-up-delay~ seconds before reading the transcript so the CLI can finish writing the just-completed turn.  When enabled, ~codex.el~ tracks the session id from hook JSON, finds the transcript under ~codex-transcript-sessions-directory~, and inserts only the missing suffix when the terminal buffer already shows the beginning of the answer.  Repair text is placed at the visible truncation point only when transcript lines match the buffer in order, or before the active prompt, so unrelated repeated lines earlier in scrollback do not suppress repair.  Inserted catch-up text is normalized toward the TUI display by removing common Markdown-only syntax such as links, emphasis markers, inline-code ticks, and code fences.  Fully rendered TUI output is treated as already reflected even when it differs from the raw Markdown transcript by styling markers, bullets, indentation, or wrapping, so catch-up does not append a duplicate raw answer.  Prefer calling ~codex-refresh-from-transcript~ explicitly to repair a known stale buffer after the transcript has settled.
...
terminal ordering through ~codex--send-command-to-buffer~.  It types the
command into that specific Codex buffer, selects the buffer's window, then
submits it by calling ~codex--terminal-send-return~ interactively.  Dollar
commands trigger Codex's skill completion UI, so they receive the additional
Return events needed to accept the skill completion and submit the resulting
prompt.
...
Before user hook handlers run, ~codex-handle-hook~ also updates the target buffer's transcript metadata.  If ~codex-transcript-catch-up-on-stop~ is enabled, Stop events schedule an internal repair of the final recorded agent message after ~codex-transcript-catch-up-delay~ seconds.
