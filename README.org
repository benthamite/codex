...
The =app-server= backend renders conversation items the way the Codex CLI does.  Command executions are folded: the command line is shown, followed by at most ~codex-app-server-max-command-output-lines~ lines of output, a ~… +N lines~ marker for the remainder, and a status line with the exit result and duration.  This avoids dumping full command output (which can be megabytes) into the buffer; the default limit is 10 lines.  Reasoning summaries render as dimmed text, file changes render as folded diffs with added and removed line faces, MCP tool calls show the tool and folded result, and web searches show the query.
